...
function address() {
	...
	$(function(){
		...
		$("#cancel").click(function() {
			var cancel = confirm("프로필 수정을 취소하시겠습니까?");
			if(cancel){
				location.href="${pageContext.request.contextPath}/admin/home.do";
				return false;
			}
			return false;
		})
...
<article class="contentWrap">
	<div class="wrap">
		<h2 class="pageTitle">프로필 수정</h2>
		
		<div class="addBox">
			<form action="adminProfile.do" method="post" enctype="multipart/form-data">
				<p>
					<label>아이디 </label>
					<input class="w395" type="text" name="id" id="id" readonly value="${libInfo.lbId }"/>
				</p>
				<p>
					<label>이 름</label>
					<input class="w395" type="text" name="name" id="name" value="${libInfo.lbName}"/>
					<i class="fas fa-feather-alt"></i>
					<span class="error">한글과 영문 대 소문자를 사용하세요. (특수기호, 공백 사용 불가)</span>
				</p>
				<p>
					<label>비밀번호</label>
					<input class="w395" type="password" name="pass" value="${libInfo.lbPass}"/>
					<i class="fas fa-feather-alt"></i>
				...
				<p>
					...
					<input type="date" class="w395" name="birthday" id="dateField" min="1900-01-01" value="<fmt:formatDate value='${libInfo.lbBirthDay}' pattern='yyyy-MM-dd'/>">
					<i class="fas fa-feather-alt"></i>
					<span class="error">생년월일을 선택해주세요.</span>
				</p>
				<p>
					<label>휴대전화</label>
					<input class="w395" type="text" name="phone" value="${libInfo.lbTel }"/>
					<i class="fas fa-feather-alt"></i>
					<span class="error">(-)을 포함한 번호를 입력하세요.</span>
				</p>
				<p>
					<label>주소</label> 
					<input type="text" name="zipCode" id="zip" value="${libInfo.lbZip.zipCode }">
					<button class="btnOrange plsSearch" type="button" id="btn">검색</button>
					<i class="fas fa-feather-alt"></i>
					<span class="error">주소를 검색해주세요.</span>
				</p>
				<p>
					<input class="w395" type="text" name="baseAddress" id="base" value="${libInfo.lbBassAd} "> 
				</p>
				<p>
					<input class="w395" type="text" name="detailAddress" id="detail" value="${libInfo.lbDetailAd}">
				</p>
				
				<p>
					<label>회원 이미지</label>
					<span class="LibImg">
						<c:if test="${libInfo.libImgPath !=null }">
							[현재 이미지: ${libInfo.libImgPath }]<br>
						</c:if>
						<c:if test="${libInfo.libImgPath == null }">
							등록된 이미지가 없음
						</c:if>
					</span><br><br>
					<input type="file" name="LibImgPath" class="loadImgBtn" onchange="imageURL(this);"/>
				</p>
				<div class="submitBtn">
		...
		<div class="getImg">
			<c:choose>
				<c:when test="${libInfo.libImgPath !=null }">
					<img class="loadImg" src="${pageContext.request.contextPath }/upload/${libInfo.libImgPath}"/>
				</c:when>
				<c:when test="${libInfo.libImgPath ==null }">
