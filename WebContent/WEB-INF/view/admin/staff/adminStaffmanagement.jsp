...
<script>
	$(function() {
		$(".libAdd").click(function() {
			location.href = "${pageContext.request.contextPath}/admin/staff/staffAdd.do";
		})
	})
</script>
		...
		<div class="listWrap">
			<div class="searchBox">
				<form action="management.do" method="post">
					<input class="rdoName" type="radio" name="schType" value="사서이름"
						checked /> 사서이름 <input class="rdoId" type="radio" name="schType"
						value="사서ID" /> 사서ID <input type="text" name="schText"
				...
				<div class="addBtn">
					<button class="libAdd btnAqua">사서 등록</button>
				</div>
				...
				<div id="result">
					<table id="libList">
						...
						<c:if test="${libListNull}">
							<tr>
								<td class="searchNone" colspan="9">검색되는 사서가 없습니다.</td>
							</tr>
						</c:if>
						<c:if test="${libListNull == null }">
							<c:forEach var="libList" items="${libList}">
								<tr>
									...
									<td class="address textStyle1">
											<span class="textLeft">${libList.lbZip} ${libList.lbBassAd }${libList.lbDetailAd }</span>
									</td>
									<td class="phone textStyle1">${libList.lbTel }</td>
									<td class="title">
									<c:if test="${libList.title.titleNo ==0}">
											<span class="dftBlue fontSW">총관리자</span>
										</c:if>
										 <c:if test="${libList.title.titleNo ==1}">
											<span>사서</span>
										</c:if></td>
									<td class="joinDt"><fmt:formatDate
											value="${libList.joinDate}" pattern="yyyy-MM-dd" /></td>
									<td class="wdrCdt"><c:if test="${libList.workCdt ==1}">
											<span>재직</span>
										</c:if> <c:if test="${libList.workCdt ==0}">
											<span class="orange fontSW">퇴사</span>
										</c:if></td>
									...
									<td class="mgn">
										...
										<a class="wdrCdtRight btnPurple"
										href="${pageContext.request.contextPath }/admin/staff/staffUpdate.do?id=${libList.lbId }">수정</a>
									</td>
								</tr>
							</c:forEach>
						</c:if>
